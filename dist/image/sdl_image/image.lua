...
if true then	--sdl.SDL_BYTE_ORDER == sdl.SDL_BIG_ENDIAN then
...
function Image:init(w,h,ch)
	...
	if type(w) == 'string' then
		...
		if loadSurface == nil then error("failed to load image") end

		local surface = sdl.SDL_ConvertSurface(loadSurface, rgbaPixelFormat, sdl.SDL_SWSURFACE)
		--local surface = sdl.SDL_DisplayFormatAlpha(loadSurface)
...
-- notice - images are in BGRA format
